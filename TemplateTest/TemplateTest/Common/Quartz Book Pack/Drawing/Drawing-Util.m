...
void DrawStringCenteredInRect(NSString *string, NSDictionary * textConfig, UIColor *color, CGRect rect)
{
    ...
    CGSize stringSize = [string sizeWithAttributes:textConfig];
    ...
    [color set];
    [string drawInRect:target withAttributes:textConfig];
...
{
    PushDraw(^{
        CGRect imageRect = RectAroundCenter(CGPointMake(rect.size.width/2, rect.size.height/2),img.size);
