...
- (void)layoutSubviews
{
    ...
    CGFloat w = kPhotoWith;
    CGFloat h = kPhotoHeight;
    
    [self.subviews enumerateObjectsUsingBlock:^(UIButton *btn, NSUInteger idx, BOOL *stop) {
        
        long rowIndex = idx / perRowImageCount;
        int columnIndex = idx % perRowImageCount;
        CGFloat x = columnIndex * (w + kPhotoSpaceX);
        CGFloat y = rowIndex * (h + kPhotoSpaceY);
        btn.frame = CGRectMake(x, y, w, h);
    }];

    self.frame = CGRectMake(kRate(15), kPhotoTitleY, kScreenWidth - 2*kRate(15), totalRowCount * (kPhotoSpaceY + h));
}
