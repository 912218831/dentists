...
- (instancetype)initWithFrame:(CGRect)frame {
    frame = CGRectMake(0, 0, kScreenWidth, 45);
    self = [super initWithFrame:frame];
    self.backgroundColor = UIColorFromRGB(0xacbbc2);//JKRColor(243, 243, 243, 1.0);
...
- (void)setIsEditing:(BOOL)isEditing {
    ...
    if (_isEditing) {
        [UIView animateWithDuration:0.2 animations:^{
            self.searchTextField.left = 10;
            self.rightButton.left = kScreenWidth - 38 - 40;
            self.backgroundImageView.width = kScreenWidth - 20 - 40;
            self.cancelButton.left = kScreenWidth - 40;
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.searchTextField.left = kScreenWidth * 0.5 - 60;
            self.rightButton.left = kScreenWidth - 38;
            self.backgroundImageView.width = kScreenWidth - 20;
            self.cancelButton.left = kScreenWidth;
        } completion:^(BOOL finished) {
...
- (void)layoutSubviews {
    ...
    style.alignment = NSTextAlignmentLeft|NSTextAlignmentJustified;
    // 解决占位字垂直居中问题
    style.minimumLineHeight = self.searchTextField.font.lineHeight;
    self.searchTextField.attributedPlaceholder = [[NSAttributedString alloc]initWithString:self.placeholder attributes:@{NSForegroundColorAttributeName:COLOR_999999,NSFontAttributeName:FONT(13),NSParagraphStyleAttributeName:style}];
}

- (UIImageView *)backgroundImageView {
    if (!_backgroundImageView) {
        _backgroundImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"widget_searchbar_textfield"]];
        _backgroundImageView.frame = CGRectMake(10, 7, kScreenWidth - 20, 32);
    }
...
- (UITextField *)searchTextField {
    if (!_searchTextField) {
        _searchTextField = [[JKRSearchTextField alloc] initWithFrame:CGRectMake(kScreenWidth * 0.5 - 60, 0, kScreenWidth * 0.5 + 20 - 38, 45)];
...
- (UIButton *)rightButton {
    if (!_rightButton) {
        ...
        _rightButton.frame = CGRectMake(kScreenWidth - 38, 9, 28, 28);
    }
    return _rightButton;
}
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton setTitleColor:CD_Text33 forState:UIControlStateNormal];
